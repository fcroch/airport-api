...
async function sendWhatsAppAlert(phone, message) {
    ...
    try {
        console.log(`[ALERTA WHATSAPP] Simulando envio para ${phone}: ${message}`);
        /* DESCOMENTE ESTE BLOCO QUANDO ASSINAR SUA API
        await fetch(API_URL, {
            method: 'POST',
...
// Criar nova sessão (BLINDADA CONTRA CAMPOS VAZIOS)
app.post('/api/meetups', async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.execute(
            ...
            VALUES (?, ?, ?, ?, ?, 'active')`,
            [tracking_code, passenger_id, flight_status, terminal || 'T1', arrival_gate || 'A Definir']
        );
...
app.patch('/api/meetups/:code/join', async (req, res) => {
    ...
    try {
        ...
        await pool.execute('UPDATE MeetupSessions SET seeker_id = ? WHERE id = ?', [seeker_id, session.id]);
        res.json({ message: 'Conectado', seeker_id, passenger_id: session.passenger_id });
    } catch (error) {
        res.status(500).json({ error: 'Erro ao entrar na sessão' });
    }
});

// Atualizar Status & Gatilho do WhatsApp
app.patch('/api/meetups/:code/status', async (req, res) => {
    ...
    try {
        ...
        await pool.execute('UPDATE MeetupSessions SET flight_status = ? WHERE id = ?', [flight_status, session_id]);
        ...
        if (flight_status === 'No Saguão' || flight_status === 'Desembarcando') {
            ...
            if (seekerRows.length > 0 && seekerRows[0].phone) {
                sendWhatsAppAlert(seekerRows[0].phone, `⚠️ AeroMeet: O Passageiro [${code}] mudou o status para: ${flight_status}`);
            }
        }
